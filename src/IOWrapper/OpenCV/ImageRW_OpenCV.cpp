...
namespace dso
{
	...
	namespace IOWrap
	{
		...
		MinimalImageB *readVideoBW_8U(int id, int width, int height)
		{
			...
			if (!cap->read(m))
			{
				printf("cv::imread could not read image %d! this may segfault. \n", id);
				return 0;
			}
			// cv::resize(m, m, cv::Size(1280, 720), 0, 0, CV_INTER_LINEAR);
			// cv::resize(m, m, cv::Size(width, height), 0, 0, CV_INTER_LINEAR);
		...
		MinimalImageB3 *readVideoRGB_8U(int id, int width, int height)
		{
			...
			memcpy(img->data, m.data, m.rows * m.cols*3);
			return img;
		}
