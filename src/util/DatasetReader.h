...
public:
	ImageFolderReader(std::string path, std::string calibFile, std::string gammaFile, std::string vignetteFile)
	{
		...
		isFfmpeg = (path.length() > 6 && path.substr(path.length() - 6) == "ffmpeg");
		isVid = (path.length() > 4 && (path.substr(path.length() - 4) == ".mp4" || path.substr(path.length() - 4) == ".MP4"));
		...
		{
			int capsucc = IOWrap::createCap(path);
			printf("reading MP4 file!!!! %d\n", capsucc);

			if (capsucc == -1)
			{
				printf("error reading .mp4 file!\n");
				exit(1);
			}
		}
		else
		...
		{
			printf("got %d files in %s!\n", getNumImages(), path.c_str());
		}
	}
	~ImageFolderReader()
	{
...
private:
	MinimalImageB *getImageRaw_internal(int id, int unused)
	{
		if (isVid)
		{
			return IOWrap::readVideoBW_8U(id, width, height);
		}
		else if (!isZipped)
		{
			// CHANGE FOR ZIP FILE
	...
	MinimalImageB3 *getColorImageRaw_internal(int id, int unused)
	{
		if (isVid)
		{
			return IOWrap::readVideoRGB_8U(id, width, height);
		}
		else if (!isZipped)
		{
			// CHANGE FOR ZIP FILE
